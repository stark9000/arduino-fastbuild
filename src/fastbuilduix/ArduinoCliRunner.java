...
final class ArduinoCliRunner {
    ...
    static String run(String arduinoCliPath, String configFile, String... args) throws CliException {
        return run(arduinoCliPath, configFile, null, args);
    }

    /**
     * Same as run(), but if processHolder is non-null, the actual started
     * Process is stored into it right after launch - so a caller running this
     * on a background thread can later call processHolder.get().destroyForcibly()
     * to actually stop it. Plain Thread.interrupt() does NOT work for this:
     * readAll() below blocks on plain stream I/O, which is not interruptible
     * (unlike Process.waitFor(), Thread.sleep, etc) - a thread blocked there
     * simply ignores an interrupt and keeps waiting for the process to finish
     * on its own. Destroying the process directly is what actually unblocks it
     * (the killed process's stdout closes, so the blocked read hits EOF).
     */
    static String run(String arduinoCliPath, String configFile, java.util.concurrent.atomic.AtomicReference<Process> processHolder, String... args) throws CliException {
